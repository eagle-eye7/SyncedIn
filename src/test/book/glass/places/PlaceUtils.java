...
public final class PlaceUtils {
	 ...
	 * @throws IOException
	 */
	public static Place getRandom(String kind, String type, double lat,
	...
	private static List<Place> performSearch(String kind, String type,
			double lat, double lng) throws IOException {
		HttpTransport httpTransport = new NetHttpTransport();
		HttpRequestFactory hrf = httpTransport.createRequestFactory();

		String paramsf = "%s?location=%s,%s&radius=1500&&types=%s|%s&key=%s";
		String params = String.format(paramsf, SearchData.BASE_URL, lat, lng,
				type, kind, AuthUtils.API_KEY);
	...
	private static Place populateDetails(Place place) throws IOException {
		// grab more restaurant details
		HttpTransport httpTransport = new NetHttpTransport();
		HttpRequestFactory hrf = httpTransport.createRequestFactory();

		String paramsf = "%s?reference=%s&key=%s";
		String params = String.format(paramsf, DetailsData.BASE_URL,
				place.getReference(), AuthUtils.API_KEY);
		GenericUrl url = new GenericUrl(params);
		HttpRequest hreq = hrf.buildGetRequest(url);
		HttpResponse hres = hreq.execute();
